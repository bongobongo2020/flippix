...
namespace FlipPix.UI.ViewModels
{
    public class StoryImageGeneratorViewModel : INotifyPropertyChanged
    {
        ...
        private async Task<string> ProcessQueueItemAsync(StoryPromptItem item, string inputImagePath, System.Threading.CancellationToken cancellationToken)
        {
            ...
            // Load workflow (2511API.json - Qwen image edit workflow)
            var workflowPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "workflow", "2511API.json");
        ...
        private JsonElement UpdateWorkflowParameters(JsonElement workflow, string inputImageName, string promptText)
        {
            ...
            // Add SaveImage node to save the output to disk
            // Node 9 will take output from Node 8 (VAEDecode)
            var saveImageNode = new
            {
                inputs = new
                {
            ...
            workflowDict["9"] = JsonSerializer.SerializeToElement(saveImageNode);

            // Update image input (node 213 - LoadImage)
            ...
            // Update positive prompt (node 153 - TextEncodeQwenImageEditPlus)
            ...
            // Update negative prompt (node 154 - TextEncodeQwenImageEditPlus)
            ...
            // Update sampler settings (node 3 - KSampler)
            ...
            try
            {
                ...
                while (retryCount < maxRetries && !images.Any())
                {
                    ...
                    else
                    {
                        ...
                        AddLog($"Found {recentFiles.Count} recent PNG files in output directory");
                        ...
                        else
                        {
                            ...
                            // Fallback: look for ANY PNG file modified in the last 10 minutes
                            if (retryCount >= 5) // After 5 retries, look for older files too
                            {
                                var olderFiles = Directory.GetFiles(comfyUIOutputDir, "*.png")
                                    .Select(f => new FileInfo(f))
                                    .Where(f => (DateTime.Now - f.LastWriteTime).TotalMinutes < 10)
                                    .OrderByDescending(f => f.LastWriteTime)
                                    .ToList();

                                AddLog($"Fallback: Found {olderFiles.Count} PNG files in last 10 minutes");
