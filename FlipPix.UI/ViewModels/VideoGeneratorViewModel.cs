...
namespace FlipPix.UI.ViewModels
{
    ...
    ///
    /// MainVM has no tab of its own any more, but the window's status bar still binds the
    /// flat names it forwards (StatusBarMessage, ComfyUIServer, ComfyUIPort), so the
    /// pass-through region below stays. Every tab binds through its own VM property
    /// instead (DataContext={Binding Scail2VM} and friends) and needs no forwarding here.
    /// </summary>
    public partial class VideoGeneratorViewModel : ObservableObject, IDisposable
    {
        ...
        /// composed by the model, not derived from a depth map afterwards.
        /// </summary>
        ...
        public void SetImagePath(string imagePath)
        {
            ...
            SelectedTabIndex = 4; // MiniMax I2V tab
        }
