...
namespace FlipPix.UI.ViewModels.Video
{
    ...
    public partial class H3EnsembleViewModel
    {
        ...
        /// still duplicated. What is in the prompt box is what Add to Queue enqueues, so a shorter honest
        /// chain beats a full-length one that renders the same file six times.
        /// </summary>
        ...
        /// <summary>
        /// The corrective appended to the user message on the retry: the beats already spent, named as spent,
        /// plus an instruction to enumerate what is left before writing. Quoting them back is the part that
        /// matters — the model cannot see its own last reply, so "do not repeat yourself" is unenforceable.
        /// </summary>
            ...
            IReadOnlyList<string> clips, IReadOnlyDictionary<int, int> repeats)
        {
            ...
            sb.Append("\nBefore writing a single clip: list every remaining beat of the story in order and ")
              ...
              .Append("reversal and the aftermath are four beats, not one. You also have a cast of several ")
              .Append("people: a beat seen from another character's side, in another part of the location, ")
              .Append("is a different beat. Every clip must show an action that appears in no other clip.");
