...
namespace FlipPix.UI.ViewModels.Video
{
    ...
    public partial class MiniMaxFflfViewModel : VideoProcessingBaseViewModel
    {
        ...
        // Detail pass: a 1.5x latent upscale re-sampled at 0.4 denoise. The loop needs the same answer
        // twice — once for its own pass, once to know whether the frames handed to it are already 1.5x.
        ...
        ///
        /// <para>Seeded from a measured run on 21 Aug: a four-clip 40s take at 0.4 MP (1.47 weighted Gpx)
        /// rendered in 15m27s, which is 604 s/Gpx — slower than the I2V tab's 513, because an FL2VA pass
        /// carries a second conditioning image and this graph resizes the carried context on top. Only
        /// the first minute of a run leans on it; from the first sampler step the estimate is re-derived
        /// from the run itself.</para>
        /// </summary>
        private static double _secondsPerGpx = 604;
        ...
        /// canvas area × frames, ×2.25 when the detail pass is on because its 1.5× is linear. Clips run
        /// one after another, so the peak is set by the largest clip, not by the total.
        /// </summary>
            ...
            get
            {
                ...
                if (ram > FrameRamOverLimitGb)
                    return $"⚠ {head} — bigger than anything that has finished on this box. Fewer clips " +
                           "per take is the biggest lever; then shorter clips, or no detail pass.";
        ...
        private bool TryAddToQueue()
        {
            ...
            if (IsMemoryOverLimit && !(_isQueueingFolder && _folderMemoryConfirmed))
            {
                var proceed = MessageBox.Show(
                    ...
                    "Fewer clips per take is the biggest lever; then shorter clips, lower quality, or " +
                    "no detail pass.\n\n" +
        ...
        private async Task<string?> PreflightAsync(MiniMaxFflfQueueItem item, CancellationToken token)
        {
            ...
            if (frameRamGb * PreflightPeakFactor > ramFree)
            {
                ...
                if (frameRamGb * PreflightPeakFactor > ramFree)
                    return $"Not enough host RAM even after unloading models: the take needs roughly "
                           ...
                           + "lever; then shorter clips, lower quality, or no detail pass.";
            }
            ...
            if (gpx > gpxCeiling)
            {
                ...
                return $"Too large for the GPU: {item.Megapixels:0.0} MP at {item.AspectRatio} resolves to "
                       ...
                       + $"{fits:0.00} MP ({fw}×{fh}), shorten the longest clip, or turn off the detail pass.";
            }
            ...
            try
            {
                       ...
                       (item = _queue.FirstOrDefault(x => x.ItemStatus == QueueItemStatus.Pending)) != null)
                {
                    ...
                    catch (Exception ex) when (IsServerDeath(ex))
                    {
                        ...
                        AddLog("Shorten the clips, drop one, lower the quality, or turn off the detail "
                               + "pass, then press Start.");
                        UpdateQueueStatus();
                        SaveQueueToFile();
                        break;
                    }
        ...
        /// what each of their steps is worth, and the unreported work between them. The figures come from
        /// measured MiniMax H3 runs — 8 base steps, a 4-step detail re-sample at 3.0× the cost of one, and
        /// a continuing clip at ~1.55× the base pass it extends. They only have to be roughly right: the
        /// pace is re-measured from the run itself as it goes.
        /// </summary>
        private (List<ProgressStage> Stages, double LeadUnits, double EstimatedSeconds) BuildProgressPlan(
            MiniMaxFflfQueueItem item)
        {
            const double detailStepUnits = 3.0;   // a 1.5x-linear step against a base step of the same pass
            ...
            const double rtxUnits = 4.0;          // RTX 2x, on the saved pass only
            const double leadUnits = 4.0;         // loading the model, encoding the keyframes
        ...
        /// <summary>Pixels per sampled frame: the canvas ComfyUI will really produce, times 2.25 when the
        /// detail pass is on because its 1.5× is linear.</summary>
            ...
            long runSeed, string runToken, string sink, out int pruned)
        {
            ...
            // already 32-aligned. A base that is a multiple of 64 makes 1.5x land exactly.
            SetInput(root, NodeResolution, "multiple", ResolutionMultiple);

            // ── Attention and quality switches ────────────────────────────────
