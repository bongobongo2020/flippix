...
namespace FlipPix.UI.Linux.Services
{
    ...
    public class NodeInstallerService
    {
        ...
        /// <summary>
        /// Finds the local ComfyUI install's Python interpreter. Windows layouts (portable
        /// python_embeded, venv/Scripts) are checked for cross-platform parity, then the Linux
        /// ones (venv/bin/python, .venv, system python3 — the usual Arch/venv setup).
        ...
        private string? ResolveEmbeddedPython()
        {
            ...
            var candidates = new[]
            {
                // Windows layouts (kept for parity with the WPF app)
                ...
                Path.Combine(comfy, ".venv", "Scripts", "python.exe"),
                // Linux layouts
                Path.Combine(comfy, "venv", "bin", "python"),
                Path.Combine(comfy, "venv", "bin", "python3"),
                portableRoot != null ? Path.Combine(portableRoot, "venv", "bin", "python") : null,
                Path.Combine(comfy, ".venv", "bin", "python"),
                "/usr/bin/python3",
            };
